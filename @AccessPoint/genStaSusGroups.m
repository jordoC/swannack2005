...
function obj = genStaSusGroups( obj, maxGroupSize)
    addpath ./    
    tmpSusGroups = cell(1);
    if (length(obj.susPollSets) == 0)
        obj.susGroups = cell(1);
        return; 
    end
    for set_idx = 1:length(obj.susPollSets(1,:))
        S = obj.susPollSets(set_idx,:);
        %if ~cellfun('isempty',S(1, 1))
        if ~isnumeric(S{set_idx})
            rootId = S{set_idx}.id;
            n = numel(S);
            x = 1:n;
